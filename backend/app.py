...
def load_device():
    ...
    if not isinstance(data["assets"], list) or len(data["assets"]) == 0:
        return jsonify({"error": "Il dispositivo deve avere almeno un asset"}), 400
...
def get_decision_tree(dt_id):
    ...
    if not os.path.exists(path):
        return jsonify({"error": f"Decision tree '{dt_id}' non trovato"}), 404
...
def save_session():
    ...
    if not data or "device" not in data or "results" not in data:
        return jsonify({"error": "Dati sessione non validi"}), 400
...
def download_report(session_id):
    ...
    lines = [
        "=" * 60,
        "REPORT DI CONFORMITÀ EN 18031",
        "=" * 60,
        f"Dispositivo : {device.get('name', 'N/A')}",
        f"Modello     : {device.get('model', 'N/A')}",
        f"OS          : {device.get('os', 'N/A')}",
        f"Data        : {s['saved_at'][:10]}",
        "=" * 60,
        "",
        "RISULTATI PER ASSET:",
        "",
    ...
    for asset in device.get("assets", []):
        ...
        if not asset_results:
            lines.append("    Nessuna valutazione eseguita.")
    ...
    lines += [
        ...
        "RIEPILOGO",
        "=" * 60,
        f"  Totale valutazioni : {total}",
        f"  ✅ PASS            : {passed}",
        f"  ❌ FAIL            : {failed}",
        f"  ➖ NOT APPLICABLE  : {na}",
        "",
        "Generato da: EN18031 Compliance PoC — Coderius Group",
        "=" * 60,
    ]
